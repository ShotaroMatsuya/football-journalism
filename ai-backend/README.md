...
```bash
ai-backend$ sam build --use-container
```
...
```bash
ai-backend$ sam logs -n StockCheckerFunction --stack-name ai-backend --tail
```
...
```bash
ai-backend$ pip install -r tests/requirements.txt --user
# unit test
ai-backend$ python -m pytest tests/unit -v
# integration test, requiring deploying the stack first.
# Create the env variable AWS_SAM_STACK_NAME with the name of the stack we are testing
ai-backend$ AWS_SAM_STACK_NAME=<stack-name> python -m pytest tests/integration -v
```
...
```bash
aws cloudformation delete-stack --stack-name ai-backend
```

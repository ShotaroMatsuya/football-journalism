...
```bash
firebase-auth-example$ sam build
```
...
```bash
firebase-auth-example$ sam local invoke HelloWorldFunction --event events/event.json
```

The SAM CLI can also emulate your application's API. Use the `sam local start-api` to run the API locally on port 3000.

```bash
firebase-auth-example$ sam local start-api
firebase-auth-example$ curl http://localhost:3000/
```
...
```bash
firebase-auth-example$ sam logs -n HelloWorldFunction --stack-name firebase-auth-example --tail
```
...
```bash
firebase-auth-example$ cd hello-world
...
```bash
aws cloudformation delete-stack --stack-name firebase-auth-example
```

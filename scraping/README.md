...
```bash
scraping$ sam build --use-container
```
...
```bash
scraping$ sam logs -n StockCheckerFunction --stack-name scraping --tail
```
...
```bash
scraping$ pip install -r tests/requirements.txt --user
# unit test
scraping$ python -m pytest tests/unit -v
# integration test, requiring deploying the stack first.
# Create the env variable AWS_SAM_STACK_NAME with the name of the stack we are testing
scraping$ AWS_SAM_STACK_NAME=<stack-name> python -m pytest tests/integration -v
```
...
```bash
aws cloudformation delete-stack --stack-name scraping
```
